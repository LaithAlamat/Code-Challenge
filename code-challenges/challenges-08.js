...
const getEntries = (obj) => {
  ...
  for (let i = 0; i < Object.entries(obj).length; i++) {
    ...
    if (typeof Object.entries(obj)[i][1] == "object") {
      typeof Object.entries(obj)[i][1].forEach((element) => {
        editedStr += `${element},`;
      });
      output.push(`${Object.entries(obj)[i][0]}: ${editedStr.slice(0, -1)}`);
    } else {
      output.push(`${Object.entries(obj)[i][0]}: ${Object.entries(obj)[i][1]}`);
    }
  }
  return output;
};
